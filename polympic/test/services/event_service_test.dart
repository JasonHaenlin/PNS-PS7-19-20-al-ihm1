...
main(){
  group('Event fetch test',(){
    test('test to fetch the data from api', () async {
      ...
      when(client.get('https://polympic.otakedev.com/events')).thenAnswer((_) async => http.Response(json.encode(EVENT_MOCK).toString(),200));
      expect(await eventService.getData(), const TypeMatcher<List<EventModel>>());
    });
  });
}